...
\begin{figure}[t]
    ...
    \vspace{-4mm}
    \caption{Comparison of ReID methods. (a) Entangled global feature fusion. (b) \textit{Our method}: MLLM-guided decoupling of identity and clothing.}
    \label{fig:figure1}
\end{figure}
...
For instance, models employing ViT~\cite{dosovitskiyImageWorth16x162021} capture fine-grained details, while methods leveraging CLIP~\cite{yaoFILIPFinegrainedInteractive2021} learn a well-aligned joint space.
...
However, a primary obstacle is that existing datasets lack specific annotations separating identity attributes from clothing details. 
...
\subsection{Feature Decoupling for Person Re-Identification}
...
This motivates our design: BDAM supplies factor-aware representations, while a Mamba fusion module models long-horizon interactions with linear complexity, enabling precise alignment without the memory and efficiency bottlenecks of standard Transformers.
...
We employ an MLLM (specifically, GPT-4) to automatically generate fine-grained identity and clothing descriptions for pedestrians, reducing the burden of manual annotation and enriching the available supervision. 
...
\subsubsection{Loss and Prompting} Table~\ref{tab:ablation_loss} validates our multi-objective loss. 
...
To verify robustness against clothing variations, we conduct an ablation on the cloth-changing RSTPReid dataset, focusing on two key designs: \textit{Explicit Decoupling} via HSIC and \textit{Explicit Exclusion} of clothing features. We compare the full model against two variants: \textit{w/o Decoupling Loss} and \textit{w/ Clothing Fusion} (intentionally re-introducing clothing features into the fusion module).

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{robustness_ablation.pdf}
    \caption{\textbf{Robustness analysis on RSTPReid.} The performance degradation in variants lacking decoupling or retaining clothing features validates the efficacy of our interference-removal strategy.}
    \label{fig:robustness_ablation}
\end{figure}

Results in Fig.~\ref{fig:robustness_ablation} reveal two key insights. 
First, regarding \textit{clothing features as interference}, the \textit{w/ Clothing Fusion} variant suffers significant degradation across all metrics. 
This empirically confirms that in cloth-changing scenarios, visual clothing features act as confounding noise. 
By structurally discarding them, our method ensures reliance on stable identity semantics. 

Second, regarding how \textit{decoupling ensures precision}, the \textit{w/o Decoupling Loss} variant shows a notable drop in Rank-1 accuracy, despite relatively stable Rank-10 performance. 
This disparity indicates that while entangled features suffice for coarse retrieval, the HSIC constraint is indispensable for the semantic purity required for precise top-rank matching. 
Thus, our robustness stems from the synergy of mathematically enforcing independence and structurally blocking interference.
