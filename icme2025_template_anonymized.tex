...
Text-to-Image Person Re-Identification (T2I-ReID) retrieves a target pedestrian from a large-scale image gallery given a natural-language description~\cite{dingSemanticallySelfAlignedNetwork2021,shenUserNegotiationsAuthenticity2025a}.
This technology is valuable for video surveillance~\cite{bukhariLanguageVisionBased2023,shiHierarchicalIdentityLearning2025}, intelligent security~\cite{galiyawalaPersonRetrievalSurveillance2021}, public safety, and social media.
Despite recent progress, practical deployment remains challenging due to image factors (e.g., pose, viewpoint, illumination) obscuring identity cues and a persistent modality gap hampering fusion\cite{liOneShotKnowledgeTransfer2025,daiDiffusionbasedSyntheticData2025}.
These issues are exacerbated at the fine-grained level, where semantic alignment is particularly difficult\cite{zuoUFineBenchTowardsTextbased2024,wenDashFusionDualstreamAlignment2025}.
...
For instance, models employing ViT~\cite{dosovitskiyImageWorth16x162021} capture fine-grained details, while methods leveraging CLIP~\cite{yaoFILIPFinegrainedInteractive2021,jiaAdaptiveIlluminationInvariantSynergistic2025} learn a well-aligned joint space.
...
This separation is enforced by a multi-objective loss strategy, including an alignment loss and an HSIC-based orthogonality constraint.
Furthermore, we pioneer the integration of the Mamba as an efficient fusion module, adept at capturing long-range cross-modal dependencies with linear complexity.
...
We evaluate on three benchmarks: CUHK-PEDES~\cite{liPersonSearchNatural2017}, ICFG-PEDES~\cite{zhuDSSLDeepSurroundingsperson2021}, and RSTPReid~\cite{dingSemanticallySelfAlignedNetwork2021}, adhering to their official identity-based splits. 
...
Driven by asymmetric losses, the BDAM's dimension-level gate ($g_{\text{dis}}$) exhibits a clear identity-centric bias, with identity weights ($\mu=0.61$) significantly exceeding clothing weights ($\mu=0.38$). 
In contrast, the instance-level fusion gate ($g_{\text{fus}}$) maintains equilibrium between modalities (means 0.52 vs. 0.48), preventing modality collapse. 
...
To verify the source of robustness against appearance changes, we conduct an ablation on the cloth-changing RSTPReid dataset. 
We focus on two core mechanisms: \textit{Explicit Decoupling} via the HSIC constraint and \textit{Explicit Exclusion} of transient features during fusion. 
We compare the full model against two variants: \textit{w/o Decoupling Loss} and \textit{w/ Transient Fusion} (intentionally re-introducing decoupled transient features, such as clothing, into the fusion module).

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{robustness_ablation.pdf}
    \vspace{-4mm}
    \caption{\textbf{Robustness analysis on RSTPReid.} The performance degradation in variants lacking decoupling constraints or actively fusing transient features validates the efficacy of our interference-removal strategy.}
    \vspace{-4mm}
    \label{fig:robustness_ablation}
\end{figure}

Results in Fig.~\ref{fig:robustness_ablation} reveal two critical insights into feature disentanglement.
First, regarding structural exclusion: The \textit{w/ Transient Fusion} variant suffers significant degradation across all metrics. 
This confirms that in cloth-changing scenarios, transient visual attributes act as confounding noise rather than complementary information. 
Retaining these features forces the model to overfit to salient but unreliable cues (e.g., clothing color), leading to negative transfer when appearance changes.
By structurally discarding them, our method enforces reliance on stable identity semantics.
%
Second, regarding mathematical decoupling: The \textit{w/o Decoupling Loss} variant shows a notable drop in Rank-1 accuracy, despite relatively stable Rank-10 performance. 
This disparity indicates that without the orthogonality constraint, the identity subspace suffers from \textit{residual entanglement}—it still implicitly leaks transient information.
While these impure features suffice for coarse retrieval, the strict HSIC constraint is indispensable for achieving the semantic purity required for precise top-rank matching.
In summary, our robustness stems from the synergy of mathematically enforcing feature independence and structurally blocking interference.
